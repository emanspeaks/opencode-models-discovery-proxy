...
export class ModelLoadingMonitor {
  ...
  startMonitoring(modelId: string, baseURL: string): void {
    ...
    console.info(`[opencode-models-discovery-proxy] Started monitoring model loading`, { modelId, baseURL })
  ...
  private async checkLoadingProgress(modelId: string, baseURL: string, getModelsFn?: () => Promise<string[]>): Promise<void> {
    try {
      ...
      if (isLoaded) {
        ...
        this.stopMonitoring(modelId)
        console.info(`[opencode-models-discovery-proxy] Model loading completed`, {
          modelId,
  ...
  private updateState(
    ...
    if (currentState.status !== status) {
      if (status === 'loaded') {
        console.info(`[opencode-models-discovery-proxy] Model loading completed`, { modelId })
      } else if (status === 'error') {
        console.warn(`[opencode-models-discovery-proxy] Model loading failed`, { modelId, error })
      } else if (status === 'loading') {
        console.info(`[opencode-models-discovery-proxy] Model loading started`, { modelId })
      }
    }
  }
